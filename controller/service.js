...
exports.newService = async function (req, res, next) {
    try {
        ...
        if (!req.body.title || !req.body.description || !req.body.expert || !req.body.slug || !req.body.metatitle || !req.body.ogmetatitle || !req.body.metadescription || !req.body.ogmetadescription || !req.body.metatag || !req.body.ogmetaimage || !req.body.category || !req.body.icontype || !req.body.order || !req.body.status || !req.body.image) {
            throw new Error("please enter valid data")
        }
        const checkCategory = await SERVICECATEGORY.findOne({ Name: req.body.category })
        if (!checkCategory) {
...
exports.updateService = async function (req, res, next) {
    try {
        ...
        const checkCategory = await SERVICECATEGORY.findOne({ Name: data.category })
        if (!checkCategory) {
            throw new Error('please select valid category')
        }   
        if(data.status){
...
exports.deleteService = async function (req, res, next) {
  ...
  exports.searchService = async function (req, res, next) {
    ...
    const data = await SERVICE.find({ title: { "$regex": req.params.name, '$options': 'i' } }) 
     res.status(200).json({
        status: "Successfull",
